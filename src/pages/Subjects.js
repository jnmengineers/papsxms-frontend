...
function Subjects() {
    ...
    const sectionForGrade = {
        PG: 'PRE_SCHOOL', PP1: 'PRE_SCHOOL', PP2: 'PRE_SCHOOL',
        G1: 'LOWER_PRIMARY', G2: 'LOWER_PRIMARY', G3: 'LOWER_PRIMARY',
        G4: 'UPPER_PRIMARY', G5: 'UPPER_PRIMARY', G6: 'UPPER_PRIMARY',
        G7: 'JUNIOR_SCHOOL', G8: 'JUNIOR_SCHOOL', G9: 'JUNIOR_SCHOOL'
    };
    ...
    const fetchSubjects = async (skipSeed = false) => {
        try {
            ...
            setLoading(false);
            if (!skipSeed && response.data.length === 0) {
                await seedSubjects();
            }
        } catch (err) {
    ...
    const handleQuickAdd = async (subjectName, gradeLevel) => {
        ...
        if (exists) {
            setSuccessMsg(`"${subjectName}" already in pool for ${gradeLevel}`);
        ...
        try {
            await api.post('/api/subjects', {
                subjectName,
                subjectCode: subjectName.replace(/[^A-Za-z]/g, '').substring(0, 6).toUpperCase(),
                gradeLevel
            });
            fetchSubjects(true);
        ...
        for (const name of section.subjects) {
            ...
            if (!exists) {
                try {
                    await api.post('/api/subjects', {
                        subjectName: name,
                        subjectCode: name.replace(/[^A-Za-z]/g, '').substring(0, 6).toUpperCase(),
                        gradeLevel
                    });
                    added++;
                } catch (err) {}
    ...
    const getSectionForGrade = (gradeLevel) => sectionForGrade[gradeLevel] || '';

    const sections = ['PRE_SCHOOL', 'LOWER_PRIMARY', 'UPPER_PRIMARY', 'JUNIOR_SCHOOL'];
    const sectionGrades = {
        PRE_SCHOOL: ['PG', 'PP1', 'PP2'],
        LOWER_PRIMARY: ['G1', 'G2', 'G3'],
        UPPER_PRIMARY: ['G4', 'G5', 'G6'],
        JUNIOR_SCHOOL: ['G7', 'G8', 'G9']
    };
    const sectionNames = {
        PRE_SCHOOL: '🟣 Pre-School (PG, PP1, PP2)',
        LOWER_PRIMARY: '🔵 Lower Primary (G1, G2, G3)',
        UPPER_PRIMARY: '🟠 Upper Primary (G4, G5, G6)',
        JUNIOR_SCHOOL: '🟢 Junior Secondary School (G7, G8, G9)'
    };
    const sectionColors = {
        PRE_SCHOOL: '#6f42c1',
        LOWER_PRIMARY: '#2E75B6',
        UPPER_PRIMARY: '#fd7e14',
        JUNIOR_SCHOOL: '#20c997'
    };
        ...
        <div style={styles.container}>
            ...
            <div style={styles.content}>
                <div style={styles.header}>
                    ...
                    <button onClick={() => { setShowForm(!showForm); setEditingSubject(null); setActiveTab('pool'); }} style={styles.addBtn}>
                ...
                <div style={styles.tabs}>
                    <button onClick={() => setActiveTab('pool')} style={{
                        ...styles.tab,
                        backgroundColor: activeTab === 'pool' ? '#1F3864' : 'white',
                        color: activeTab === 'pool' ? 'white' : '#1F3864'
                    }}>
                        📋 Subject Pool ({subjects.length})
                    </button>
                    <button onClick={() => setActiveTab('quick')} style={{
                        ...styles.tab,
                        backgroundColor: activeTab === 'quick' ? '#1F3864' : 'white',
                        color: activeTab === 'quick' ? 'white' : '#1F3864'
                    }}>
                        ⚡ Quick Add by Section
                    </button>
                </div>

                {/* TAB 1 — Subject Pool grouped by section */}
                {activeTab === 'pool' && (
                    <>
                                ...
                                <table style={styles.table}>
                                    ...
                                    <tbody>
                                        {sections.map(section => {
                                            const sectionFiltered = filtered.filter(s =>
                                                sectionGrades[section].includes(s.gradeLevel)
                                            );
                                                ...
                                                <React.Fragment key={section}>
                                                    ...
                                                    {sectionFiltered.map((subject, index) => (
                                                        <tr key={subject.subjectId} style={index % 2 === 0 ? styles.trEven : styles.trOdd}>
                                                            ...
                                                            <td style={styles.td}>
                                                                <span style={styles.codeBadge}>{subject.subjectCode}</span>
                                                            </td>
                                                            <td style={styles.td}>
                                                                <span style={{
                                                                    backgroundColor: sectionColors[section],
                ...
                {/* TAB 2 — Quick Add by Section */}
                {activeTab === 'quick' && (
                    <div>
                        <p style={styles.quickHint}>
                            Click a subject tile to add it to the pool. Green = already in pool.
                            Use <strong>Add All</strong> to add the entire section at once.
                        </p>
                            ...
                            <div key={sectionKey} style={styles.sectionBlock}>
                                <div style={{...styles.sectionHeader, backgroundColor: section.color}}>
                                    ...
                                    <div style={styles.sectionRight}>
                                        ...
                                        <button onClick={() => handleAddAllForSection(sectionKey)} style={styles.addAllBtn}>
                                            ➕ Add All
                                        </button>
                                    </div>
                                </div>
                                    ...
                                    {section.subjects.map(subjectName => {
                                        ...
                                        return (
                                            <div key={subjectName}
                                                style={{
                                                    ...styles.quickTile,
                                                    backgroundColor: inPool ? '#e8f5e9' : 'white',
                                                    color: inPool ? '#28a745' : '#333',
                                                    border: inPool ? '2px solid #28a745' : `2px solid ${section.color}`,
                                                ...
                                                <span style={{...styles.quickTileStatus, color: inPool ? '#28a745' : section.color}}>
                                                    {inPool ? 'In Pool' : 'Click to Add'}
                                                </span>
                                            </div>
                                        );
                                    })}
...
const styles = {
    ...
    quickTile: { padding: '15px 10px', borderRadius: '8px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '5px', transition: 'all 0.2s' },
