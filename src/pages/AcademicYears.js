...
const YearForm = ({ formData, setFormData, onSubmit, onCancel, submitLabel }) => (
    <form onSubmit={onSubmit} style={styles.inlineForm}>
        <div style={styles.formGrid}>
            ...
            <div style={styles.formGroup}>
                <label style={styles.label}>🗓️ Start Date</label>
            ...
            <div style={styles.formGroup}>
                <label style={styles.label}>🗓️ End Date</label>
...
function AcademicYears() {
    ...
    const fetchClasses = async () => {
        try { const r = await api.get('/api/classes'); setClasses(r.data); } catch (err) {}
    };

    const handleEdit = (year) => {
        if (editingYear?.yearId === year.yearId) { setEditingYear(null); return; }
        setEditingYear(year);
        setFormData({ yearLabel: year.yearLabel, term: year.term, startDate: year.startDate, endDate: year.endDate, isActive: year.isActive });
    ...
    const handleSubmitAdd = async (e) => {
        ...
        try {
            ...
            setShowAddForm(false); setFormData(emptyForm); fetchYears();
            setSuccessMsg('✅ Academic year added!'); setTimeout(() => setSuccessMsg(''), 3000);
    ...
    const handleSubmitEdit = async (e) => {
        ...
        try {
            ...
            setEditingYear(null); setFormData(emptyForm); fetchYears();
            setSuccessMsg('✅ Academic year updated!'); setTimeout(() => setSuccessMsg(''), 3000);
        } catch (err) { setError('Failed to update academic year'); }
    };

    const handleDelete = async (id) => {
        try { await api.delete(`/api/academic-years/${id}`); fetchYears(); setDeleteConfirm(null); }
    ...
    const openPromotion = () => {
        const pairs = classes.map(cls => {
            ...
            const suggestedNext = nextGrade
                ? classes.find(c => c.gradeLevel === nextGrade && (streamSuffix ? c.stream?.charAt(0).toUpperCase() === streamSuffix : !c.stream))
                    || classes.find(c => c.gradeLevel === nextGrade)
                : null;
            return {
                fromClassId: String(cls.classId), fromClassName: cls.className, fromGrade: gradeLevel,
                toClassId: suggestedNext ? String(suggestedNext.classId) : '', toClassName: suggestedNext?.className || '',
                skip: !nextGrade
            };
    ...
    const handlePromote = async () => {
        ...
        if (!validPairs.length) { setError('No valid promotion pairs selected'); return; }
        setPromoting(true);
        ...
        for (let i = 0; i < validPairs.length; i++) {
            ...
            try {
                ...
                const classStudents = studentsRes.data.filter(s => String(s.schoolClass?.classId) === String(pair.fromClassId));
    ...
    return (
        <div style={styles.container}>
            ...
            <div style={styles.content}>
                <div style={styles.header}>
                    <div>
                        ...
                        <p style={styles.subtitle}>
                            {activeYear
                                ? <span>Active: <strong>{activeYear.yearLabel} — Term {activeYear.term}</strong> ({activeYear.startDate} → {activeYear.endDate})</span>
                                : 'No active term set'}
                        </p>
                    </div>
                    <div style={styles.headerBtns}>
                        <button onClick={openPromotion} style={styles.promoteBtn}>🎓 Promote Students</button>
                    ...
                    <div style={styles.modalOverlay}>
                        <div style={{ backgroundColor:'white', padding:'25px 30px', borderRadius:'10px', maxWidth:'380px', width:'90%' }}>
                            ...
                            <div style={{ display:'flex', gap:'10px', justifyContent:'flex-end' }}>
                                ...
                                <button onClick={() => handleDelete(deleteConfirm.yearId)} style={{ ...styles.cancelBtn, backgroundColor:'#dc3545' }}>🗑️ Delete</button>
                            </div>
                        </div>
                    </div>
                )}
                    ...
                    <div style={styles.modalOverlay}>
                        <div style={styles.modal}>
                            ...
                            <p style={styles.modalSubtitle}>
                                Review the promotion mapping below. Students will move to the target class keeping all their results.
                                G9 students graduate and are not promoted.
                            </p>

                            {promotionProgress && (
                                <div style={{ margin:'15px 25px 0' }}>
                                    <div style={{ height:'8px', backgroundColor:'#e9ecef', borderRadius:'4px', overflow:'hidden', marginBottom:'6px' }}>
                                        <div style={{ height:'100%', backgroundColor:'#28a745', width:`${(promotionProgress.done/promotionProgress.total)*100}%`, transition:'width 0.3s' }} />
                                    </div>
                                    <p style={{ fontSize:'12px', color:'#666', margin:0 }}>
                                        Promoting {promotionProgress.done}/{promotionProgress.total} — ✅ {promotionProgress.success} done
                ...
                {loading ? <p style={{ textAlign:'center', padding:'40px', color:'#666' }}>⏳ Loading...</p> :
                                    ...
                                    {termsList.sort((a,b) => a.term-b.term).map(year => (
                                        <div key={year.yearId}>
                                            <div style={{ ...styles.termCard, borderTop:`4px solid ${year.isActive?'#28a745':'#ddd'}`, outline: editingYear?.yearId === year.yearId ? '2px solid #2E75B6' : 'none' }}>
                                                ...
                                                <div style={{ display:'flex', alignItems:'center', gap:'6px', fontSize:'12px', color:'#666', marginBottom:'12px' }}>
                                            ...
                                            {editingYear?.yearId === year.yearId && (
                                                <div style={{ backgroundColor:'white', borderRadius:'0 0 8px 8px', padding:'15px', border:'2px solid #2E75B6', borderTop:'none', marginTop:'-2px' }}>
                                                    ...
                                                    <YearForm formData={formData} setFormData={setFormData}
                                                        onSubmit={handleSubmitEdit} onCancel={handleCancelEdit} submitLabel="✅ Update" />
                                                </div>
                                            )}
                                        </div>
                                    ))}
