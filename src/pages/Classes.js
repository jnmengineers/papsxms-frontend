...
function Classes() {
    ...
    const [editingClass, setEditingClass] = useState(null); // inline edit
    ...
    const fetchStudentsByClass = async (classId) => {
        ...
        try {
            ...
            setStudents(response.data.filter(s => String(s.schoolClass?.classId) === String(classId)));
    ...
    const handleSectionChange = (sectionValue) => {
        ...
        setFormData(prev => ({ ...prev, section: sectionValue, meanTarget: section ? section.target : '', gradeLevel: '', className: '' }));
    };
    ...
    const handleSubmitAdd = async (e) => {
        ...
        try {
            await api.post('/api/classes', {
                className: formData.className, stream: formData.stream || null,
                gradeLevel: formData.gradeLevel, section: formData.section,
    ...
    const handleSubmitEdit = async (e) => {
        e.preventDefault();
        try {
            await api.put(`/api/classes/${editingClass.classId}`, {
                className: formData.className, stream: formData.stream || null,
                gradeLevel: formData.gradeLevel, section: formData.section,
                meanTarget: parseFloat(formData.meanTarget)
            });
    ...
    // Shared class form
    const ClassFormFields = ({ onSubmit, onCancel, submitLabel }) => (
            ...
            <div style={styles.formGrid}>
                ...
                <div style={styles.formGroup}>
                    ...
                    <input style={{...styles.input, backgroundColor: '#f8f9fa', color: '#1F3864', fontWeight: 'bold'}}
                        value={formData.meanTarget ? `${formData.meanTarget}%` : ''} readOnly
                    ...
                    <>
                        ...
                        {view === 'grades' && (
                            <div>
                                ...
                                {classes.length === 0 && (
                                    <div style={styles.emptyState}><div style={styles.emptyIcon}>🏫</div><h3>No Classes Yet</h3><p>Click + Add Class to get started</p></div>
                                )}
                            </div>
                        )}

                        {/* VIEW 2 — Stream Tiles with inline edit below */}
                                            ...
                                            <div key={cls.classId}>
                                                ...
                                                <div style={{
                                                    ...
                                                    <div style={styles.streamTop} onClick={() => handleClassClick(cls)}>
                                                        ...
                                                        <div style={styles.streamTarget}>🎯 Target: {cls.meanTarget}%</div>
                                                        <div style={styles.viewStudentsBtn}>👥 View Students →</div>
                                                    </div>
                                                    <div style={styles.streamActions}>
                                                        ...
                                                        <div style={styles.streamBtns}>
                                                            ...
                                                            <button onClick={e => { e.stopPropagation(); handleDelete(cls.classId); }} style={styles.deleteBtn}>Delete</button>
                                                        </div>
                                                    </div>
                                                </div>

                                                {/* ── Inline Edit Form — opens below the stream card ── */}
                                                {isEditing && (
                                                    <div style={styles.inlineEditCard}>
                                                        <div style={styles.inlineEditHeader}>
                                                            <h4 style={styles.inlineEditTitle}>✏️ Editing: {cls.className}</h4>
                        ...
                        {view === 'students' && selectedClass && (
                            <div>
                                ...
                                {loadingStudents ? <p style={styles.centerMsg}>⏳ Loading students...</p> : students.length === 0 ? (
                                    <div style={styles.emptyState}><div style={styles.emptyIcon}>👥</div><h3>No Students Yet</h3><p>Go to Students page to add students to this class</p></div>
                                ) : (
                                        ...
                                        {students.map((student, index) => (
                                            <div key={student.studentId} style={styles.studentCard}>
                                                ...
                                                <div style={styles.studentInfo}>
                                                    ...
                                                    <span style={{ ...styles.studentGender, backgroundColor: student.gender === 'Male' ? '#2E75B6' : '#e83e8c' }}>{student.gender}</span>
                                                </div>
...
const styles = {
    ...
    gradeTile: { backgroundColor: 'white', padding: '20px 15px', borderRadius: '8px', textAlign: 'center', cursor: 'pointer', boxShadow: '0 2px 4px rgba(0,0,0,0.08)', border: '1px solid #eee' },
